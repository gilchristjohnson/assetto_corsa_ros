...
class _Logger(Protocol):
    ...
    def info(self, msg: str) -> None:  # pragma: no cover - protocol
        """Log an informational message."""

    def warning(self, msg: str) -> None:  # pragma: no cover - protocol
    ...
    def _open(self) -> socket.socket:
        try:
            ...
            self._logger.warning(
                f"Failed to bind Assetto UDP socket on {host}:{port}; restarting bridge: {exc}"
            )
            raise AssettoSocketError("Unable to bind Assetto UDP socket") from exc

        host, port = self._address
        self._logger.info(f"Assetto UDP listening on {host}:{port}")
        return sock

    def _fail(self, action: str, exc: OSError) -> None:
        self._logger.warning(
            f"Socket error while {action}; restarting bridge: {exc}"
        )
        self.close()
